...
export async function getOrCreateDeviceUuid(): Promise<string> {
  ...
  if (!id) {
    // Use crypto.getRandomValues if available (React Native Hermes 0.71+),
    // otherwise fall back to the timestamp + Math.random approach.
    if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
      ...
      // Fallback: not cryptographically strong but acceptable for a device ID
